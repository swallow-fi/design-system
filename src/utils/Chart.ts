...
export type ChartOption = echarts.EChartOption & {
...
export default class Chart {
    ...
    protected option: ChartOption | null;
    ...
    public render(option: ChartOption | null): void {
        if (
            this.chart === null ||
        ...
        if (option === null) {
            console.error("차트 옵션이 존재하지 않습니다");
            return;
        }
    ...
    protected setTooltipContent(params: any) {
        ...
        if (param.seriesType === "candlestick") {
            return `
                  <div><span style="font-size: 11px; color: #333;">${param.axisValue
                }</span></div>
            ...
            const array = params.map((localParam: any) => {
                return `
                      ...
                      <span style="width: 8px; height: 8px; background: ${localParam.color
                    }; display: inline-block;"></span>
                      <span style="font-size: 11px; color: white;">${localParam.seriesName
                    }: </span>
